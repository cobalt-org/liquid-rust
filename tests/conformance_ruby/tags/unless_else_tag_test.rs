...
fn test_unless_else() {
    ...
    assert_template_result!(" YES ", r#"{% unless "foo" %} NO {% else %} YES {% endunless %}"#);
}

#[test]
fn test_unless_in_loop() {
    assert_template_result!("23", "{% for i in choices %}{% unless i %}{{ forloop.index }}{% endunless %}{% endfor %}", v!({"choices": [1, nil, false]}));
}
