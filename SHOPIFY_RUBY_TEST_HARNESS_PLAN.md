...
- The harness should use an in-process Ruby replacement `liquid` gem backed by a Rust native extension.
...
- Upstream checkout path will be `/Users/ahmed/Desktop/marfoof/shopify-liquid`
...
The upstream Ruby suite is the single source of truth for compatibility.
...
- when an upstream test asserts Ruby-visible internals, that observed API or object graph is part of the compatibility target for this project
- mirrored or translated Rust tests may be added for speed, but they do not replace the upstream result
- if Rust-native tests and upstream Ruby tests disagree, the upstream Ruby test wins
...
Use an in-process replacement gem architecture from the start.
...
3. Rust native extension
   ...
   - owns parser, renderer, runtime state, and other engine responsibilities
...
The replacement gem should own the Ruby API surface expected by upstream tests, including:
...
The Rust extension should own the engine behavior, including:
...
- resource limit accounting
- template and context state representation behind Ruby-visible wrappers or proxies where needed

### Bridge Responsibilities

The bridge between Ruby and Rust should provide:
...
- Ruby callback dispatch for filters, tags, blocks, drops, `liquid_method_missing`, `to_liquid`, `to_liquid_value`, and proc or lambda-backed values
- error and warning translation
...
- `/Users/ahmed/Desktop/marfoof/shopify-liquid`
