...
/// `Result` extension methods for adapting third party errors to `CompilerError`.
pub trait ResultCompilerChainExt<T, E> {
    /// Create an `CompilerError` with `E` as the cause.
    fn chain(self, msg: &'static str) -> Result<T>;
    /// Create an `CompilerError` with `E` as the cause.
    fn chain_with<F>(self, msg: F) -> Result<T> where F: FnOnce() -> String;
}

/// `Result` convenience extension methods for working with `CompilerError`.
impl<T, E> ResultCompilerChainExt<T, E> for result::Result<T, E>
    where E: error::Error + 'static + Send
{
    fn chain(self, msg: &'static str) -> Result<T> {
        self.map_err(|err| CompilerError::with_msg(msg).cause(err))
    }
...
impl<T> ResultCompilerExt<T> for Result<T> {
...
// Guts of `CompilerError` here to keep `CompilerError` small to avoid bloating the size of `Result<T>` in the
...
const ERROR_DESCRIPTION: &str = "parse error";
