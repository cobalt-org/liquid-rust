...
pub fn tokenize(text: &str) -> Result<Vec<Element>> {
    ...
    for block in split_blocks(text) {
        if let Some(caps) = TAG.captures(block) {
            blocks.push(Element::Tag(granularize(caps.get(1).map(|x| x.as_str()).unwrap_or(""))?,
                                     block.to_owned()));
        } else if let Some(caps) = EXPRESSION.captures(block) {
            blocks.push(Element::Expression(granularize(caps.get(1)
                                                            .map(|x| x.as_str())
                                                            .unwrap_or(""))?,
                                            block.to_owned()));
        } else {
...
mod test {
    ...
    fn test_tokenize() {
        assert_eq!(tokenize("{{hello 'world'}}").unwrap(),
                   vec![Element::Expression(vec![Token::Identifier("hello".to_owned()),
                                                 Token::StringLiteral("world".to_owned())],
                                            "{{hello 'world'}}".to_owned())]);
        assert_eq!(tokenize("{{hello.world}}").unwrap(),
                   vec![Element::Expression(vec![Token::Identifier("hello.world".to_owned())],
                                            "{{hello.world}}".to_owned())]);
        assert_eq!(tokenize("{{ hello 'world' }}").unwrap(),
                   vec![Element::Expression(vec![Token::Identifier("hello".to_owned()),
                                                 Token::StringLiteral("world".to_owned())],
        ...
        assert_eq!(tokenize("wat\n{{hello 'world'}} test").unwrap(),
                   vec![Element::Raw("wat\n".to_owned()),
                        Element::Expression(vec![Token::Identifier("hello".to_owned()),
                                                 Token::StringLiteral("world".to_owned())],
                                            "{{hello 'world'}}".to_owned()),
                        Element::Raw(" test".to_owned())]);
        assert_eq!(tokenize("wat \n {{-hello 'world'-}} test").unwrap(),
                   vec![Element::Raw("wat".to_owned()),
                        Element::Expression(vec![Token::Identifier("hello".to_owned()),
                                                 Token::StringLiteral("world".to_owned())],
                                            " \n {{-hello 'world'-}} ".to_owned()),
                        Element::Raw("test".to_owned())]);
    ...
    fn test_granularize() {
        ...
        assert_eq!(granularize("'test' == \"me\"").unwrap(),
                   vec![Token::StringLiteral("test".to_owned()),
                        Token::Comparison(ComparisonOperator::Equals),
        ...
        assert_eq!(granularize("abc : \"1, '2', 3, 4\"").unwrap(),
                   vec![Token::Identifier("abc".to_owned()),
                        Token::Colon,
                        Token::StringLiteral("1, '2', 3, 4".to_owned())]);
    }
}
