...
module Liquid
  class Lexer
    ...
    COLON = [:colon, ":"].freeze
    COMMA = [:comma, ","].freeze
    COMPARISON_NOT_EQUAL = [:comparison, "!="].freeze
    ...
    TWO_CHARS_COMPARISON_JUMP_TABLE = [].tap do |table|
      ...
      table["!".ord] = [].tap do |sub_table|
        sub_table["=".ord] = COMPARISON_NOT_EQUAL
        sub_table.freeze
      end
      table.freeze
    end
    ...
    NEXT_MATCHER_JUMP_TABLE = [].tap do |table|
      "a".upto("z") do |char|
        table[char.ord] = [:id, IDENTIFIER].freeze
        table[char.upcase.ord] = [:id, IDENTIFIER].freeze
      end
      table["_".ord] = [:id, IDENTIFIER].freeze

      "0".upto("9") do |char|
        table[char.ord] = [:number, NUMBER_LITERAL].freeze
      end
      ...
      def tokenize(ss)
        ...
        until ss.eos?
          ...
          if (special = SPECIAL_TABLE[peeked])
            ...
            if type && (token = ss.scan(pattern))
              output << if type == :id && token == "contains" && output.last&.first != :dot
                COMPARISON_CONTAINS
              else
                [type, token]
              end
        ...
        raise unless error.message == "invalid byte sequence in #{ss.string.encoding}"
