...
module Liquid
  module StandardFilters
    ...
    HTML_ESCAPE_ONCE_REGEXP = /["><']|&(?!([a-zA-Z]+|(#\d+)|(#x[0-9A-Fa-f]+));)/
    ...
    def default(input, default_value = '', options = {})
      ...
      return default_value if false_check || (input.respond_to?(:empty?) && input.empty?)

      input.respond_to?(:liquid_render_value) ? input.liquid_render_value : input
    end
